...
bool is_add_to_be_fused(const ov::Output<ov::Node>& output) {
    ...
    ov::rocm_gpu::MatMulOp::BroadcastToMatrix(matrix_A_shape);
    ov::rocm_gpu::MatMulOp::BroadcastToMatrix(matrix_B_shape);
    const auto matmul_batch = std::max(ov::rocm_gpu::MatMulOp::GetMatrixNumBatches(matrix_A_shape),
                                       ov::rocm_gpu::MatMulOp::GetMatrixNumBatches(matrix_B_shape));

    auto const_shape = constant_node->get_output_shape(0);
    ov::rocm_gpu::MatMulOp::BroadcastToMatrix(const_shape);
    const auto const_batch = ov::rocm_gpu::MatMulOp::GetMatrixNumBatches(const_shape);
