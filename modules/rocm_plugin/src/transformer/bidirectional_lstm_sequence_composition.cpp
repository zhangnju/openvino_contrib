...
std::shared_ptr<ov::Node> find_concat(const Output<Node>& node, std::shared_ptr<Node>& transpose) {
    for (const auto& in : node.get_target_inputs()) {
        ...
        if (ov::as_type_ptr<ov::op::v0::Concat>(current_node) ||
            ov::as_type_ptr<ov::rocm_gpu::nodes::ConcatOptimized>(current_node)) {
            return current_node;
        }
...
bool bidirectional_lstm_sequence_MIOPEN_optimized(const std::shared_ptr<ov::op::v5::LSTMSequence>& lstm_sequence_bidirectional) {
    // Original OpenVINO:
    ...
    using MajorFormat = ov::rocm_gpu::nodes::LSTMSequenceOptimized::MajorFormat;
    ...
    auto new_lstm_sequence_bidirectional = std::make_shared<ov::rocm_gpu::nodes::LSTMSequenceOptimized>(
...
bool ConvertBidirectionalLSTMSequenceToBidirectionalLSTMSequenceOptimized::run_on_model(const std::shared_ptr<ov::Model>& f) {
    ...
    for (const auto& op : f->get_ordered_ops()) {
        if (is_bidirectional(op)) {
            was_updated |= bidirectional_lstm_sequence_MIOPEN_optimized(ov::as_type_ptr<ov::op::v5::LSTMSequence>(op));
        }
    }
    return was_updated;
}
