...
bool convolution_backprop_data_with_padding(Matcher &m) {
    static_assert(std::is_same_v<TBaseConvolution, ov::op::v1::ConvolutionBackpropData> ||
                      ...
                      std::is_same_v<TBaseConvolution, ov::rocm_gpu::nodes::FusedConvBackpropData>,
    ...
    if constexpr (std::is_same_v<TBaseConvolution, ov::rocm_gpu::nodes::FusedConvBackpropData>) {
...
FusedConvBackpropDataAsymPaddingTransformation::FusedConvBackpropDataAsymPaddingTransformation() {
    ...
    const auto conv = wrap_type<ov::rocm_gpu::nodes::FusedConvBackpropData>();

    matcher_pass_callback callback = [](Matcher &m) {
        return convolution_backprop_data_with_padding<ov::rocm_gpu::nodes::FusedConvBackpropData>(m);
    };
    const auto m = std::make_shared<Matcher>(conv, matcher_name);
    register_matcher(m, callback);
}
