...
#include "memory_manager/rocm_memory_pool.hpp"
...
class rocmInferRequest : public ov::ISyncInferRequest {
public:
    using Ptr = std::shared_ptr<rocmInferRequest>;

    explicit rocmInferRequest(const std::shared_ptr<const CompiledModel>& compiled_model);
    ~rocmInferRequest() = default;
...
private:
    std::shared_ptr<const CompiledModel> get_rocm_model();
