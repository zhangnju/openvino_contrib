...
FakeQuantizeOp::FakeQuantizeOp(const CreationContext &context,
    ...
    kernel_ = kernel::FakeQuantize{
        convertDataType<ov::rocm_gpu::kernel::Type_t>(element_type), output_size, max_threads_per_block, levels};
}

rocmGraphCompatibility FakeQuantizeOp::GetrocmGraphCompatibility() const { return rocmGraphCompatibility::FULL; }
