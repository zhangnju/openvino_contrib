...
DetectionOutputOp::DetectionOutputOp(const CreationContext& context,
    ...
    : OperationBase{context, node, std::move(inputIds), std::move(outputIds)}, element_type_{node.get_input_element_type(0)} {
    ...
    if (node.inputs().size() == 5) {
        kernel_ =
            std::make_optional<kernel::DetectionOutput>(convertDataType<ov::rocm_gpu::kernel::Type_t>(element_type_),
...
rocmGraphCompatibility DetectionOutputOp::GetrocmGraphCompatibility() const { return rocmGraphCompatibility::FULL; }
