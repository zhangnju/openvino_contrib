...
GatherElementsOp::GatherElementsOp(const CreationContext& context,
    ...
    indices_element_size_ = node->get_input_element_type(1).size();
...
WorkbufferRequest GatherElementsOp::GetWorkBufferRequest() const {
    // Two immutable buffers: data_strides and out_strides (each ndim_ int64 values)
    const size_t strides_bytes = ndim_ * sizeof(int64_t);
    return {{strides_bytes, strides_bytes}, {}};
}

void GatherElementsOp::InitSharedImmutableWorkbuffers(const Buffers& buffers) {
    OPENVINO_ASSERT(buffers.size() == 2, "Node name: ", GetName());
    rocm::DefaultStream::stream().upload(buffers[0], data_strides_.data(), ndim_ * sizeof(int64_t));
    rocm::DefaultStream::stream().upload(buffers[1], out_strides_.data(), ndim_ * sizeof(int64_t));
}
...
void GatherElementsOp::Execute(const InferenceRequestContext& context,
    ...
    OPENVINO_ASSERT(wbs.immutable_buffers.size() == 2, "Node name: ", GetName());

    const int64_t* d_data_strides = static_cast<const int64_t*>(wbs.immutable_buffers[0].get());
    const int64_t* d_out_strides  = static_cast<const int64_t*>(wbs.immutable_buffers[1].get());
