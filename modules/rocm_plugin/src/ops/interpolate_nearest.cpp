...
std::vector<float> getScalesVector(const ov::rocm_gpu::InterpolateNearestOp::NodeOp& node) {
...
InterpolateNearestOp::InterpolateNearestOp(const CreationContext& context,
    ...
    const auto element_type = convertDataType<ov::rocm_gpu::kernel::Type_t>(node.get_input_element_type(0));
...
void InterpolateNearestOp::Execute(const InferenceRequestContext& context,
    ...
    const hipStream_t stream = context.getThreadContext().stream().get();
...
rocmGraphCompatibility InterpolateNearestOp::GetrocmGraphCompatibility() const { return rocmGraphCompatibility::FULL; }
