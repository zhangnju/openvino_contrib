...
ClamprocmOp::ClamprocmOp(const CreationContext& context,
                         const NodeOp& node,
    ...
    if (out_element_type != element_type) {
        throw_ov_exception(
            fmt::format("ClamprocmOp: output type should be the same as input type, input type: {}, output type: {}",
    ...
    kernel_ = kernel::Clamp{
        convertDataType<ov::rocm_gpu::kernel::Type_t>(element_type), max_threads_per_block, num_elements, min, max};
}

void ClamprocmOp::Execute(const InferenceRequestContext& context,
                          Inputs inputTensors,
...
rocmGraphCompatibility ClamprocmOp::GetrocmGraphCompatibility() const { return rocmGraphCompatibility::FULL; }
