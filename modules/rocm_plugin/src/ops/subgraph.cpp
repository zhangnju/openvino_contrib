...
void SubGraph::initExecuteSequence(bool isStableParams, bool isStableResults) {
    ...
    for (unsigned node_idx = 0; node_idx < orderedNodes.size(); node_idx++) {
        ...
        auto operation = OperationRegistry::getInstance().createOperation(creation_context_, node, std::move(inIds), std::move(outIds));
...
std::size_t SubGraph::GetrocmGraphsCount() const {
    if (!hasTopologyRunners()) {
        return graph_compatibility_ == rocmGraphCompatibility::NONE ? 0 : 1;
    }
    auto count = runner_ ? runner_->GetrocmGraphsCount() : static_cast<std::size_t>(0);
    for (const auto& op : exec_sequence_) {
        const auto sg = std::dynamic_pointer_cast<SubGraph>(op);
        if (sg) {
            count += sg->GetrocmGraphsCount();
        }
    }
    return count;
}
...
rocmGraphCompatibility SubGraph::GetrocmGraphCompatibility() const {
    if (!is_compatibility_analyzed_) {
        graph_compatibility_ = rocmGraphCompatibility::FULL;
        for (const auto& op : exec_sequence_) {
            auto opCompatability = op->GetrocmGraphCompatibility();
            if (opCompatability == rocmGraphCompatibility::SPECIAL) {
                graph_compatibility_ = opCompatability;
            } else if (opCompatability == rocmGraphCompatibility::NONE) {
                graph_compatibility_ = opCompatability;
                break;
            }
        }
...
void SubGraph::initializeRunner() {
    runner_ = std::make_shared<rocmGraphTopologyRunner>(creation_context_, model_, exec_sequence_, memory_manager_);
}
