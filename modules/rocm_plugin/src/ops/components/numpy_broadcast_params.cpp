...
template <typename T>
static void uploadDataToWorkbuffer(rocm::DevicePointer<void*> buffer, const std::vector<T>& v) {
...
void NumpyBroadcastParamsImpl::initWorkbuffers(const std::vector<rocm::DevicePointer<void*>>& buffers) const {
    uploadDataToWorkbuffer(rocm::DevicePointer<void*>{ib_src_strides_.requiredPtr(buffers)}, src_strides_);
    uploadDataToWorkbuffer(rocm::DevicePointer<void*>{ib_dst_strides_.requiredPtr(buffers)}, dst_strides_);
    uploadDataToWorkbuffer(rocm::DevicePointer<void*>{ib_broadcasted_dims_.requiredPtr(buffers)}, broadcasted_dims_);
}

kernel::NumpyBroadcastMapper NumpyBroadcastParamsImpl::mapper(
    const std::vector<rocm::DevicePointer<const void*>>& immutable_buffers) const {
