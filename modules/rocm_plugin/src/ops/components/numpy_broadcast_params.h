...
#include "kernels/details/numpy_broadcast_mapper.hpp"
...
public:
    ...
    virtual void initWorkbuffers(const std::vector<rocm::DevicePointer<void*>>& buffers) const = 0;

    virtual kernel::NumpyBroadcastMapper mapper(
        const std::vector<rocm::DevicePointer<const void*>>& immutable_buffers) const = 0;
};
...
public:
    ...
    void initWorkbuffers(const std::vector<rocm::DevicePointer<void*>>& buffers) const override {}
    kernel::NumpyBroadcastMapper mapper(
        const std::vector<rocm::DevicePointer<const void*>>& immutable_buffers) const override {
