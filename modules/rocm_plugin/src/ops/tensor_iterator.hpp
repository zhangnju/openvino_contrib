...
private:
    ...
    public:
        ...
        void addKernelNode(IrocmGraphInfo& info,
                           const rocm::Stream& stream,
                           rocm::DevicePointer<void*> mutableBuffer,
                           const IOperationExec::Inputs& inputTensors);

        void updateKernelNode(IrocmGraphInfo& info,
                              std::size_t index,
                              rocm::DevicePointer<void*> mutableBuffer,
                              const IOperationExec::Inputs& inputTensors,
                              int64_t iter) {
    ...
    public:
        ...
        void operator()(const rocm::Stream& stream, rocm::DevicePointer<void*> mutableBuffer) const {
            ...
            throwIfError(hipMemcpyAsync(dst, src, param_size_, hipMemcpyDeviceToDevice, stream.get()));
        }

        void addTransferNode(IrocmGraphInfo& info,
                             const rocm::Stream& stream,
                             rocm::DevicePointer<void*> mutableBuffer);
    ...
    void transferParam(const rocm::Stream& stream,
                       rocm::DevicePointer<void*> mutableBuffer,
                       const IOperationExec::Inputs& inputTensors,
                       std::int64_t iter,
                       uint64_t inputIdx,
    ...
    void transferResult(const rocm::Stream& stream,
                        rocm::DevicePointer<void*> mutableBuffer,
                        const IOperationExec::Outputs& outputTensors,
                        int64_t iter,
