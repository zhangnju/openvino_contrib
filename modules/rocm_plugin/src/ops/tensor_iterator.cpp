...
TensorIteratorOp::TensorIteratorOp(const CreationContext& context,
    ...
    for (const auto& [inputIdx, portMap] : portmap_inputs_) {
        ...
        kernel::Type_t element_type = convertDataType<ov::rocm_gpu::kernel::Type_t>(inputType);
...
rocmGraphCompatibility TensorIteratorOp::GetrocmGraphCompatibility() const {
    // This implementation is rocm graph compatible only if this is the standard TI with output only of the last
    ...
    if (!is_compatibility_analyzed_) {
        graph_compatibility_ = rocmGraphCompatibility::NONE;
        for (const auto& op : exec_sequence_) {
            auto opCompatability = op->GetrocmGraphCompatibility();
            if (opCompatability == rocmGraphCompatibility::SPECIAL || opCompatability == rocmGraphCompatibility::FULL) {
                graph_compatibility_ = rocmGraphCompatibility::SPECIAL;
                break;
            }
        }
...
void TensorIteratorOp::initializeRunner() {
    ...
    // fully rocm graph compatible
    if (std::any_of(exec_sequence_.begin(), exec_sequence_.end(), [](const auto& op) {
            return op->GetrocmGraphCompatibility() != rocmGraphCompatibility::FULL;
        })) {
...
void TensorIteratorOp::SliceLauncher::addKernelNode(IrocmGraphInfo& info,
                                                    const rocm::Stream& stream,
                                                    rocm::DevicePointer<void*> mutableBuffer,
...
void TensorIteratorOp::TransferLauncher::addTransferNode(IrocmGraphInfo& info,
                                                         const rocm::Stream& stream,
                                                         rocm::DevicePointer<void*> mutableBuffer) {
...
void TensorIteratorOp::InsertLauncher::addKernelNode(IrocmGraphInfo& info,
                                                     const rocm::Stream& stream,
                                                     rocm::DevicePointer<void*> mutableBuffer,
...
void TensorIteratorOp::CaptureSingle(InferenceRequestContext& context,
    ...
    auto& executionDelegator = context.getExecutionDelegator();
    executionDelegator.set_stream(stream);
    // auto& graphInfo = context.getrocmGraphContext().get_current_graph();
    auto& graphInfo = context.getCurrentrocmGraphInfo();
...
void TensorIteratorOp::ExecuteGraphSingle(InferenceRequestContext& context,
    ...
    OPENVINO_ASSERT(graphInfo.get_kernels_count() == slices_.size() + inserts_.size(),
                    "rocmGraphContext/TensorIteratorOp slices or inserts count incosistency");
...
void TensorIteratorOp::CaptureMulti(InferenceRequestContext& context,
    ...
    auto& bodyGraphInfo = graphPack.add(rocmGraphPack::create());
    context.setCurrentrocmGraphInfo(bodyGraphInfo);
    // Inner loop
    runner_->Capture(context, workbuffers);

    graphPack.add(rocmGraphInfo::create());
    rocm::GraphCapture capture2{stream};
    {
...
void TensorIteratorOp::ExecuteGraphMulti(InferenceRequestContext& context,
    ...
    for (int64_t iter = 0; iter < num_iterations_; ++iter) {
        ...
        context.setCurrentrocmGraphInfo(bodyContext);
...
void TensorIteratorOp::transferParam(const rocm::Stream& stream,
                                     const rocm::DevicePointer<void*> mutableBuffer,
                                     const IOperationExec::Inputs& inputTensors,
...
void TensorIteratorOp::transferResult(const rocm::Stream& stream,
                                      rocm::DevicePointer<void*> mutableBuffer,
                                      const IOperationExec::Outputs& outputTensors,
                                      const std::int64_t iter,
                                      const std::size_t resultIdx,
