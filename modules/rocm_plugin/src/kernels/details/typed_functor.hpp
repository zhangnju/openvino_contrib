...
/// rocm_type_traits_t<...>, it must be wrapped by the functor TFunctor like the next.
///
...
/// Where subscription index (type_1, type_2, type_3) is of type ov::rocm_gpu::kernel::Type_t enum.
///
...
/// And the function pointer can be accessed like the next.
///    auto func_ptr = instance[ov::rocm_gpu::kernel::Type_t::f32];
// clang-format on
...
private:
    ...
    constexpr TypedFunctor(std::index_sequence<I...>, std::index_sequence<Indx...>)
        : std::array<TFunPtr, type_count>{
              &TFunctor<rocm_type_traits_t<static_cast<Type_t>(Indx + type_t_first_value)>...,
                        rocm_type_traits_t<static_cast<Type_t>(I + type_t_first_value)>>::function...} {}
};
