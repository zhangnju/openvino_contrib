                   ...
                   IndexCollection&& outputs) {
                    if constexpr (details::isConstructibleWithNodeOpRef<TOperation>) {
                        return std::make_shared<TOperation>(
                            context, downcast<const typename TOperation::NodeOp>(node), std::move(inputs), std::move(outputs));
                    } else {
                        if constexpr (details::isConstructibleWithNodeRef<TOperation>) {
                            return std::make_shared<TOperation>(context, *node, std::move(inputs), std::move(outputs));
                        } else {
                            return std::make_shared<TOperation>(context, node, std::move(inputs), std::move(outputs));
                        }
                    }
...
public:
    explicit Register(const std::string& opName, OperationBuilder&& builder) {
        getInstance().registerOp(opName, std::move(builder));
    }
};
...
#define OPERATION_REGISTER(type, name)                                                                           \
    extern "C" {                                                                                                 \
    [[maybe_unused]] const ::ov::rocm_gpu::OperationRegistry::Register<type> openvino_rocm_op_register_##name{ \
        #name};                                                                                                  \
    }

#define OPERATION_REGISTER_FACTORY(factory, name)                                       \
    extern "C" {                                                                        \
    [[maybe_unused]] const ::ov::rocm_gpu::OperationRegistry::Register<OperationBase> \
        openvino_rocm_op_register_##name{#name, factory};                               \
    }
