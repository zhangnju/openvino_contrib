...
rocmThreadPool::rocmThreadPool(rocm::Device d, unsigned _numThreads) {
    try {
        rocmLatch latch{_numThreads};
...
rocmThreadPool::~rocmThreadPool() { stop_thread_pool(); }

void rocmThreadPool::stop_thread_pool() noexcept {
    {
...
const ThreadContext& rocmThreadPool::get_thread_context() {
    if (!contextPtr) {
        throw_ov_exception(
...
void rocmThreadPool::run(Task task) {
    {
        std::lock_guard<std::mutex> lock(mtx_);
