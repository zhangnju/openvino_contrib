...
Plugin::Plugin() {
    ...
    // When ENABLE_ROCMLIR is active, convolution uses rocMLIR directly and does NOT
    // call MIOpen for conv. We therefore do NOT restrict MIOpen solvers here.
    // The MIOPEN_DEBUG_FIND_ONLY_SOLVER workaround is not needed in rocMLIR mode.
    for (int i = 0; i < rocm::Device::count(); ++i) {
        rocm::Device device{i};
        const size_t num_concurrent_streams = max_concurrent_streams(device);
        device_thread_pool_[std::to_string(i)] = std::make_shared<rocmThreadPool>(device, num_concurrent_streams);
...
std::shared_ptr<ov::threading::ITaskExecutor> Plugin::get_stream_executor(const Configuration& config) const {
    ...
    OPENVINO_ASSERT(device_thread_pool_.count(device_id), "Couldn't find config for rocm with id ", device_id);
...
Configuration Plugin::get_full_config(const ov::AnyMap& properties, const bool throw_on_unsupported) const {
    ...
    if (properties.find(ov::device::id.name()) != properties.end()) {
        ...
        auto pos = device_id.find_last_of("ROCM.");
        if (pos != std::string::npos) {
            device_id = device_id.substr(pos + 1);
        }
    }
    OPENVINO_ASSERT(configs_.find(device_id) != configs_.end(), "Couldn't find config for rocm with id ", device_id);
...
std::shared_ptr<ov::ICompiledModel> Plugin::compile_model(const std::shared_ptr<const ov::Model>& model,
    ...
    OV_ITT_SCOPED_TASK(itt::domains::rocm_gpu, "Plugin::compile_model");

    auto full_config = get_full_config(properties);
    rocm::Device device{full_config.get_device_id()};
...
std::shared_ptr<ov::ICompiledModel> Plugin::import_model(std::istream& model_stream,
                                                         const ov::SoPtr<ov::IRemoteContext>& context,
                                                         const ov::AnyMap& properties) const {
    OV_ITT_SCOPED_TASK(itt::domains::rocm_gpu, "ov::rocm_gpu::import_model");
...
ov::SupportedOpsMap Plugin::query_model(const std::shared_ptr<const ov::Model>& model,
                                        const ov::AnyMap& properties) const {
    OV_ITT_SCOPED_TASK(itt::domains::rocm_gpu, "ov::rocm_gpu::query_model");

    auto full_config = get_full_config(properties, false);

    auto supported = ov::get_supported_nodes(model,
    [&](std::shared_ptr<ov::Model>& model) {
            transformer_.transform(rocm::Device{full_config.get_device_id()}, model, full_config);
        },
...
bool Plugin::is_operation_supported(const std::shared_ptr<ov::Node>& node, const Configuration& config) const {
    ...
    if (OperationRegistry::getInstance().hasOperation(node)) {
        ...
        const CreationContext context{rocm::Device{config.get_device_id()}, false};
...
ov::Any Plugin::get_property(const std::string& name, const ov::AnyMap& properties) const {
    ...
    if (ov::supported_properties == name) {
        ...
        for (size_t i = 0; i < rocm::Device::count(); ++i) {
        ...
        rocm::Device device{full_config.get_device_id()};
        const auto& props = device.props();
        return decltype(ov::range_for_async_infer_requests)::value_type{1, 1 ,props.asyncEngineCount};
    } else {
