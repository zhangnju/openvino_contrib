...
void* MemoryModel::deviceBufferPtr(rocm::DevicePointer<uint8_t*> devPtr, const BufferID& id) {
    if (ptrdiff_t offset = 0; offsetForBuffer(id, offset)) return devPtr.get() + offset;
    return nullptr;
}

void* MemoryModel::deviceTensorPtr(rocm::DevicePointer<uint8_t*> devPtr, const TensorID& id) {
