...
public:
    ...
    DnnBETensorDescriptor() : DnnBackendDescriptor{MIOPEN_BACKEND_TENSOR_DESCRIPTOR} {}
};
...
public:
    ...
    auto& setUniqueId(int64_t id) {
        setAttributeValue<miopenBackendAttributeName_t::MIOPEN_ATTR_TENSOR_UNIQUE_ID>(id);
        return *this;
    }
    auto& setDataType(miopenDataType_t dataType) {
        setAttributeValue<MIOPEN_ATTR_TENSOR_DATA_TYPE>(dataType);
        return *this;
    }
    template <typename T>
    auto& setShape(const std::vector<T>& shape) {
        OPENVINO_ASSERT(shape.size() <= MIOPEN_DIM_MAX);
        std::array<int64_t, MIOPEN_DIM_MAX> dimensions;
        std::copy(shape.begin(), shape.end(), dimensions.begin());
        setAttributeValues<MIOPEN_ATTR_TENSOR_DIMENSIONS>(gsl::span<int64_t>(dimensions.data(), shape.size()));
        return *this;
    }
    template <typename T>
    auto& setStrides(const std::vector<T>& strides) {
        OPENVINO_ASSERT(strides.size() <= MIOPEN_DIM_MAX);
        std::array<int64_t, MIOPEN_DIM_MAX> values;
        std::copy(strides.begin(), strides.end(), values.begin());
        setAttributeValues<MIOPEN_ATTR_TENSOR_STRIDES>(gsl::span<int64_t>(values.data(), strides.size()));
        return *this;
    }
    auto& setAlignment(int64_t byteAlignment) {
        setAttributeValue<MIOPEN_ATTR_TENSOR_BYTE_ALIGNMENT>(byteAlignment);
        return *this;
    }
    auto& setIsVirtual(bool value) {
        setAttributeValue<MIOPEN_ATTR_TENSOR_IS_VIRTUAL>(value);
        return *this;
    }
};
...
public:
    ...
    DnnBEConvolutionDescriptor() : DnnBackendDescriptor{MIOPEN_BACKEND_CONVOLUTION_DESCRIPTOR} {}
};
...
public:
    ...
    auto& setMode(miopenConvolutionMode_t mode) {
        setAttributeValue<MIOPEN_ATTR_CONVOLUTION_CONV_MODE>(mode);
        return *this;
    }
    auto& setComputeType(miopenDataType_t computeType) {
        setAttributeValue<MIOPEN_ATTR_CONVOLUTION_COMP_TYPE>(computeType);
        return *this;
    }
    auto& setNumberOfSpatialDimensions(int64_t nDims) {
        setAttributeValue<MIOPEN_ATTR_CONVOLUTION_SPATIAL_DIMS>(nDims);
        return *this;
    }
    template <typename T>
    auto& setPrePaddings(const std::vector<T>& prePaddings) {
        OPENVINO_ASSERT(prePaddings.size() <= MIOPEN_DIM_MAX);
        std::array<int64_t, MIOPEN_DIM_MAX> values;
        std::copy(prePaddings.begin(), prePaddings.end(), values.begin());
        setAttributeValues<MIOPEN_ATTR_CONVOLUTION_PRE_PADDINGS>(gsl::span<int64_t>(values.data(), prePaddings.size()));
        return *this;
    }
    template <typename T>
    auto& setPostPaddings(const std::vector<T>& postPaddings) {
        OPENVINO_ASSERT(postPaddings.size() <= MIOPEN_DIM_MAX);
        std::array<int64_t, MIOPEN_DIM_MAX> values;
        std::copy(postPaddings.begin(), postPaddings.end(), values.begin());
        setAttributeValues<MIOPEN_ATTR_CONVOLUTION_POST_PADDINGS>(
            gsl::span<int64_t>(values.data(), postPaddings.size()));
        return *this;
    }
    template <typename T>
    auto& setDilations(const std::vector<T>& dilations) {
        OPENVINO_ASSERT(dilations.size() <= MIOPEN_DIM_MAX);
        std::array<int64_t, MIOPEN_DIM_MAX> values;
        std::copy(dilations.begin(), dilations.end(), values.begin());
        setAttributeValues<MIOPEN_ATTR_CONVOLUTION_DILATIONS>(gsl::span<int64_t>(values.data(), dilations.size()));
        return *this;
    }
    template <typename T>
    auto& setFilterStrides(const std::vector<T>& strides) {
        ...
        std::array<int64_t, MIOPEN_DIM_MAX> values;
        std::copy(strides.begin(), strides.end(), values.begin());
        setAttributeValues<MIOPEN_ATTR_CONVOLUTION_FILTER_STRIDES>(gsl::span<int64_t>(values.data(), strides.size()));
        return *this;
    }
};
...
public:
    ...
    DnnBEPointwiseDescriptor() : DnnBackendDescriptor{MIOPEN_BACKEND_POINTWISE_DESCRIPTOR} {}
};
...
public:
    ...
    auto& setMathPrecision(miopenDataType_t data_type) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_MATH_PREC>(data_type);
        return *this;
    }

    auto& setClipping(double l, double u) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_RELU_LOWER_CLIP>(l);
        setAttributeValue<MIOPEN_ATTR_POINTWISE_RELU_UPPER_CLIP>(u);
        return *this;
    }

    auto& setMode(miopenPointwiseMode_t mode) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_MODE>(mode);
        return *this;
    }

    auto& setMode(miopenNanPropagation_t nan_mode) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_NAN_PROPAGATION>(nan_mode);
        return *this;
    }

    auto& setReluLowerClip(float lower_clip) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_RELU_LOWER_CLIP>(lower_clip);
        return *this;
    }
    ...
    auto& setReluUpperClip(float upper_clip) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_RELU_UPPER_CLIP>(upper_clip);
        return *this;
    }
    ...
    auto& setReluLowerClipSlope(float lower_clip_slope) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_RELU_LOWER_CLIP_SLOPE>(lower_clip_slope);
        return *this;
    }
    ...
    auto& setEluAlpha(double elu_alpha) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_ELU_ALPHA>(elu_alpha);
        return *this;
    }

    auto& setSoftplusBeta(double softplus_beta) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_SOFTPLUS_BETA>(softplus_beta);
        return *this;
    }

    auto& setSwishBeta(double swish_beta) {
        setAttributeValue<MIOPEN_ATTR_POINTWISE_SWISH_BETA>(swish_beta);
        return *this;
    }
};
...
public:
    ...
    DnnBEOperationGraphDescriptor() : DnnBackendDescriptor{MIOPEN_BACKEND_OPERATIONGRAPH_DESCRIPTOR} {}

    int64_t getEngineCount(void) const { return getAttributeValue<MIOPEN_ATTR_OPERATIONGRAPH_ENGINE_GLOBAL_COUNT>(); }

private:
    std::shared_ptr<Handle<miopenHandle_t>> dnn_handle_;
    std::vector<std::shared_ptr<rocm::DnnBackendDescriptor>> ops_;
    std::vector<miopenBackendDescriptor_t> ops_decs_;
};
...
public:
    ...
    auto& setDnnHandle(const std::shared_ptr<rocm::DnnHandle>& dnnHandle) {
        desc_->dnn_handle_ = dnnHandle;
        setAttributeValue<MIOPEN_ATTR_OPERATIONGRAPH_HANDLE>(dnnHandle->get());
        return *this;
    }

    auto& setOperations(gsl::span<miopenBackendDescriptor_t> ops) {
        setAttributeValues<MIOPEN_ATTR_OPERATIONGRAPH_OPS>(ops);
        return *this;
    }

    auto& setOperations(gsl::span<std::shared_ptr<rocm::DnnBackendDescriptor>> ops) {
        desc_->ops_ = {ops.begin(), ops.end()};
        std::transform(ops.begin(),
                       ops.end(),
                       std::back_inserter(desc_->ops_decs_),
                       [](const auto& op) -> miopenBackendDescriptor_t { return (*op).get(); });
        setAttributeValues<MIOPEN_ATTR_OPERATIONGRAPH_OPS>(desc_->ops_decs_);
        return *this;
    }
};
...
public:
    ...
    DnnBEOperationConvolutionForwardDescriptor()
        : DnnBackendDescriptor{MIOPEN_BACKEND_OPERATION_CONVOLUTION_FORWARD_DESCRIPTOR} {}
...
public:
    ...
    auto& setXDesc(const std::shared_ptr<DnnBETensorDescriptor>& tensor_desc) {
        ...
        setAttributeValue<MIOPEN_ATTR_OPERATION_CONVOLUTION_FORWARD_X>(tensor_desc->get());
        return *this;
    }
    auto& setWDesc(const std::shared_ptr<DnnBETensorDescriptor>& tensor_desc) {
        desc_->w_desc_ = tensor_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_CONVOLUTION_FORWARD_W>(tensor_desc->get());
        return *this;
    }
    auto& setYDesc(const std::shared_ptr<DnnBETensorDescriptor>& tensor_desc) {
        desc_->y_desc_ = tensor_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_CONVOLUTION_FORWARD_Y>(tensor_desc->get());
        return *this;
    }
    auto& setConvDesc(const std::shared_ptr<DnnBEConvolutionDescriptor>& conv_desc) {
        desc_->conv_desc_ = conv_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_CONVOLUTION_FORWARD_CONV_DESC>(conv_desc->get());
        return *this;
    }
    ...
    auto& setScalingParams(typename DnnBEAttrType<TypeID>::ValueType alpha,
        ...
        setAttributeValue<TypeID>(MIOPEN_ATTR_OPERATION_CONVOLUTION_FORWARD_ALPHA, alpha);
        setAttributeValue<TypeID>(MIOPEN_ATTR_OPERATION_CONVOLUTION_FORWARD_BETA, beta);
        return *this;
    }
};
...
public:
    ...
    DnnBEOperationPointwiseDescriptor() : DnnBackendDescriptor{MIOPEN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR} {}
...
public:
    ...
    auto& setXDesc(const std::shared_ptr<DnnBETensorDescriptor>& tensor_desc) {
        desc_->x_desc_ = tensor_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_POINTWISE_XDESC>(tensor_desc->get());
        return *this;
    }
    auto& setBDesc(const std::shared_ptr<DnnBETensorDescriptor>& tensor_desc) {
        desc_->b_desc_ = tensor_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_POINTWISE_BDESC>(tensor_desc->get());
        return *this;
    }
    auto& setYDesc(const std::shared_ptr<DnnBETensorDescriptor>& tensor_desc) {
        desc_->y_desc_ = tensor_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_POINTWISE_YDESC>(tensor_desc->get());
        return *this;
    }
    auto& setDxDesc(const std::shared_ptr<DnnBEConvolutionDescriptor>& conv_desc) {
        desc_->dx_desc_ = conv_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_POINTWISE_DXDESC>(conv_desc->get());
        return *this;
    }
    auto& setDyDesc(const std::shared_ptr<DnnBEConvolutionDescriptor>& conv_desc) {
        desc_->dy_desc_ = conv_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_POINTWISE_DYDESC>(conv_desc->get());
        return *this;
    }
    auto& setPwDesc(const std::shared_ptr<DnnBEPointwiseDescriptor>& tensor_desc) {
        desc_->pw_desc_ = tensor_desc;
        setAttributeValue<MIOPEN_ATTR_OPERATION_POINTWISE_PW_DESCRIPTOR>(tensor_desc->get());
        return *this;
    }
    template <miopenBackendAttributeType_t TypeID>
    auto& setScalingParams(typename DnnBEAttrType<TypeID>::ValueType alpha1,
                           typename DnnBEAttrType<TypeID>::ValueType alpha2) {
        setAttributeValue<TypeID>(MIOPEN_ATTR_OPERATION_POINTWISE_ALPHA1, alpha1);
        setAttributeValue<TypeID>(MIOPEN_ATTR_OPERATION_POINTWISE_ALPHA2, alpha2);
        return *this;
    }
};
...
public:
    ...
    DnnBEEngine() : DnnBackendDescriptor{MIOPEN_BACKEND_ENGINE_DESCRIPTOR} {}
    ...
    DnnBEEngine(DnnBEEngine&& engine)
        ...
        for (uint64_t i = 0; i < bknobs_.size(); i++) {
            bknobs_[i] = std::make_shared<DnnBackendDescriptor>(MIOPEN_BACKEND_KNOB_INFO_DESCRIPTOR);
        }

        std::array<MIOPENBackendDescriptor_t, MIOPEN_KNOB_TYPE_COUNTS> bKnobs_ =
            {};  //!< Opaque pointer to the backend knobs_
        for (std::uint32_t i = 0; i < bknobs_.size(); i++) {
            bKnobs_[i] = bknobs_[i]->get();
        }
        throwIfError(MIOPENBackendGetAttribute(get(),
                                              MIOPEN_ATTR_ENGINE_KNOB_INFO,
                                              MIOPEN_TYPE_BACKEND_DESCRIPTOR,
                                              MIOPEN_KNOB_TYPE_COUNTS,
                                              &num_knobs_,
                                              bKnobs_.data()));
        buildKnobs();
        */
    ...
    int64_t getGlobalIndex() const { return getAttributeValue<MIOPEN_ATTR_ENGINE_GLOBAL_INDEX>(); }
...
private:
    ...
    class Knob {
    public:
        Knob(miopenBackendKnobType_t type_, int64_t max, int64_t min, int64_t stride_)
        ...
        miopenBackendKnobType_t getKnobType() const { return knobType; }
    ...
    private:
        miopenBackendKnobType_t knobType = MIOPEN_KNOB_TYPE_COUNTS;
    ...
    void buildKnobs() {
        for (auto i = 0; i < num_knobs_; i++) {
            ...
            miopenBackendKnobType_t type;
            int64_t maxValue, minValue, stride, elemCount;
            throwIfError(
                MIOPENBackendGetAttribute(bKnob, MIOPEN_ATTR_KNOB_INFO_TYPE, MIOPEN_TYPE_KNOB_TYPE, 1, &elemCount, &type));
            throwIfError(MIOPENBackendGetAttribute(
                bKnob, MIOPEN_ATTR_KNOB_INFO_MAXIMUM_VALUE, MIOPEN_TYPE_INT64, 1, &elemCount, &maxValue));
            throwIfError(MIOPENBackendGetAttribute(
                bKnob, MIOPEN_ATTR_KNOB_INFO_MINIMUM_VALUE, MIOPEN_TYPE_INT64, 1, &elemCount, &minValue));
            throwIfError(
                MIOPENBackendGetAttribute(bKnob, MIOPEN_ATTR_KNOB_INFO_STRIDE, MIOPEN_TYPE_INT64, 1, &elemCount, &stride));
    ...
    //std::array<std::shared_ptr<DnnBackendDescriptor>, MIOPEN_KNOB_TYPE_COUNTS> bknobs_ = {};
...
public:
    ...
    auto& setOpEngineGraph(const std::shared_ptr<DnnBEOperationGraphDescriptor>& graph) {
        desc_->graph_ = graph;
        setAttributeValue<MIOPEN_ATTR_ENGINE_OPERATION_GRAPH>(graph->get());
        return *this;
    }
    auto& setGlobalIndex(int64_t index) {
        setAttributeValue<MIOPEN_ATTR_ENGINE_GLOBAL_INDEX>(index);
        return *this;
    }
...
public:
    ...
    DnnBEEngineConfigDescriptor() : DnnBackendDescriptor{MIOPEN_BACKEND_ENGINECFG_DESCRIPTOR} {
        /*
        for (uint64_t i = 0; i < bchoices_.size(); i++) {
            bchoices_[i] = std::make_shared<DnnBackendDescriptor>(MIOPEN_BACKEND_KNOB_CHOICE_DESCRIPTOR);
        }
        */
    }

    DnnBEEngine getEngine() const {
        auto engines = getBEDescAttributeValues<MIOPEN_ATTR_ENGINECFG_ENGINE, DnnBEEngine>();
        if (engines.size() != 1) ov::rocm_gpu::throw_ov_exception("Unexpected number of MIOPEN Backend engines");
...
private:
    ...
    //std::array<std::shared_ptr<const DnnBackendDescriptor>, MIOPEN_KNOB_TYPE_COUNTS> bchoices_{};
...
public:
    ...
    auto& setEngine(const std::shared_ptr<DnnBEEngine>& engine) {
        ...
        for (std::uint32_t i = 0; i < knobs.size(); i++) {
            miopenBackendKnobType_t type = knobs[i].getKnobType();
            int64_t value = knobs[i].getChoice();
            throwIfError(MIOPENBackendSetAttribute(
                desc_->bchoices_[i]->get(), MIOPEN_ATTR_KNOB_CHOICE_KNOB_TYPE, MIOPEN_TYPE_KNOB_TYPE, 1, &type));
            throwIfError(MIOPENBackendSetAttribute(
                desc_->bchoices_[i]->get(), MIOPEN_ATTR_KNOB_CHOICE_KNOB_VALUE, MIOPEN_TYPE_INT64, 1, &value));
            throwIfError(MIOPENBackendFinalize(desc_->bchoices_[i]->get()));
        }
        */
        auto handle = engine->get();
        throwIfError(miopenBackendSetAttribute(
            desc_->get(), MIOPEN_ATTR_ENGINECFG_ENGINE, MIOPEN_TYPE_BACKEND_DESCRIPTOR, 1, &handle));
        /*
        if (desc_->set_knobs_attr_ && desc_->num_knobs_ > 0) {
            std::array<MIOPENBackendDescriptor_t, MIOPEN_KNOB_TYPE_COUNTS> bchoices;
            for (auto i = 0; i < desc_->num_knobs_; i++) {
                bchoices[i] = desc_->bchoices_[i]->get();
            }
            throwIfError(MIOPENBackendSetAttribute(desc_->get(),
...
public:
    ...
    DnnBEEngineHeuristicsDescriptor() : DnnBackendDescriptor{MIOPEN_BACKEND_ENGINEHEUR_DESCRIPTOR} {}

    std::vector<std::shared_ptr<DnnBEEngineConfigDescriptor>> getEngineConfigs() {
        return getBEDescAttributeValues<MIOPEN_ATTR_ENGINEHEUR_RESULTS, DnnBEEngineConfigDescriptor>();
    }
...
public:
    ...
    auto& setOpGraph(const std::shared_ptr<DnnBEOperationGraphDescriptor>& graph) {
        graph_ = graph;
        setAttributeValue<MIOPEN_ATTR_ENGINEHEUR_OPERATION_GRAPH>(graph->get());
        return *this;
    }
    auto& setMode(miopenBackendHeurMode_t mode) {
        setAttributeValue<MIOPEN_ATTR_ENGINEHEUR_MODE>(mode);
        return *this;
    }
...
public:
    ...
    DnnBEExecutionPlan() : DnnBackendDescriptor{MIOPEN_BACKEND_EXECUTION_PLAN_DESCRIPTOR} {}

    int64_t getWorkspaceSize() const { return getAttributeValue<MIOPEN_ATTR_EXECUTION_PLAN_WORKSPACE_SIZE>(); }
    std::shared_ptr<DnnBEEngineConfigDescriptor> getConfigDesc() const { return config_desc_; }

private:
    std::shared_ptr<const rocm::DnnHandle> dnn_handle_;
...
public:
    ...
    auto& setEngineConfig(const std::shared_ptr<DnnBEEngineConfigDescriptor>& config) {
        ...
        setAttributeValue<MIOPEN_ATTR_EXECUTION_PLAN_ENGINE_CONFIG>(config->get());
        return *this;
    }
    auto& setDnnHandle(const rocm::DnnHandle& dnnHandle) {
        setAttributeValue<MIOPEN_ATTR_EXECUTION_PLAN_HANDLE>(dnnHandle.get());
        return *this;
    }
    auto& setDnnHandle(const std::shared_ptr<rocm::DnnHandle>& dnnHandle) {
        desc_->dnn_handle_ = dnnHandle;
 ...
 * Passed to `MIOPENBackendExecute()`.
 */
class DnnBEVariantPack : public DnnBackendDescriptor {
public:
    friend class DnnBEVariantPackBuilder;

    DnnBEVariantPack() : DnnBackendDescriptor{MIOPEN_BACKEND_VARIANT_PACK_DESCRIPTOR} {}
};
...
public:
    ...
    auto& setTensorPointers(gsl::span<const int64_t> uids, gsl::span<const void*> ptrs) {
        setAttributeValues<MIOPEN_ATTR_VARIANT_PACK_UNIQUE_IDS>(uids);
        setAttributeValues<MIOPEN_ATTR_VARIANT_PACK_DATA_POINTERS>(ptrs);
        return *this;
    }
    auto& setWorkspase(const void* workspace) {
        setAttributeValue<MIOPEN_ATTR_VARIANT_PACK_WORKSPACE>(workspace);
        return *this;
    }
};
