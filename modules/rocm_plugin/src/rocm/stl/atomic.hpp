...
struct AtomicOperations<AtomicScope::BlockWide> {
    template <typename T>
    __device__ static T load(const T& value) noexcept {
        return rocm::atomicLoad_block(&value);
    }
    template <typename T>
    __device__ static void store(T& value, const T& val) noexcept {
...
struct AtomicOperations<AtomicScope::SystemWide> {
    template <typename T>
    __device__ static T load(const T& value) noexcept {
        return rocm::atomicLoad_system(&value);
    }
    template <typename T>
    __device__ static void store(T& value, const T& val) noexcept {
...
template <typename T>
using DeviceAtomic = Atomic<T, rocm::AtomicScope::DeviceWide>;

template <typename T>
using BlockAtomic = Atomic<T, rocm::AtomicScope::BlockWide>;

template <typename T>
using SystemAtomic = Atomic<T, rocm::AtomicScope::SystemWide>;
