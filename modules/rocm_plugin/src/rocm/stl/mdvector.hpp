...
public:
    ...
    __host__ explicit MDSpan(const rocm::Stream& stream, size_t capacity, void* data, TExtents... extents)
        : capacity_{capacity},
          sizes_{data, extents...},
          data_{static_cast<void*>(static_cast<char*>(data) + sizes_.size_bytes()), extents..., capacity} {
        stream.memset(rocm::DevicePointer<void*>{sizes_.data()}, 0, sizes_.size_bytes());
    }
