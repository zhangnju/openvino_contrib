...
#include "rocm_eager_topology_runner.hpp"
#include "rocm_graph_topology_runner.hpp"
#include "rocm_itt.hpp"
#include "rocm_operation_registry.hpp"
#include "rocm_perf_counts.hpp"
#include "rocm_plugin.hpp"
#include "memory_manager/rocm_immutable_memory_block_builder.hpp"
#include "memory_manager/rocm_memory_manager.hpp"
...
#include "transformer/rocm_graph_transformer.hpp"

namespace {
static constexpr const char* rocm_stream_executor_name = "rocmStreamExecutor";
static constexpr const char* rocm_exclusive_executor = "rocmExecutor";
static constexpr const char* rocm_callback_executor_name = "rocmCallbackExecutor";
}  // namespace
...
CompiledModel::CompiledModel(const std::shared_ptr<const ov::Model>& model,
    ...
    : ov::ICompiledModel(model, plugin, nullptr, nullptr),
      ...
      rocm_stream_executor_(std::move(wait_executor)),
      loaded_from_cache_(loaded_from_cache),
      use_hip_graph_{get_property(ov::rocm_gpu::use_hip_graph.name()).as<bool>() &&
                      !get_property(ov::enable_profiling.name()).as<bool>()},
      number_of_hip_graphs_{0} {
    try {
        compile_model(model);
...
void CompiledModel::init_executor() {
    ...
    config_.streams_executor_config_ =
        ov::threading::IStreamsExecutor::Config{rocm_stream_executor_name, static_cast<int>(memory_pool_->Size())};
    ...
    if (config_.is_exclusive_async_requests()) {
        set_task_executor(get_plugin()->get_executor_manager()->get_executor(rocm_exclusive_executor));
    } else {
        set_task_executor(get_plugin()->get_executor_manager()->get_idle_cpu_streams_executor(streams_executor_config));
    }
    set_callback_executor(get_plugin()->get_executor_manager()->get_idle_cpu_streams_executor({rocm_callback_executor_name}));
}

CompiledModel::~CompiledModel() {
    get_plugin()->get_executor_manager()->clear(rocm_stream_executor_name);
    get_plugin()->get_executor_manager()->clear(rocm_callback_executor_name);
}

void CompiledModel::compile_model(const std::shared_ptr<const ov::Model>& model) {
    rocm::Device device{config_.get_device_id()};
    ...
    if (model->is_dynamic()) {
        throw_ov_exception("Dynamic models are not supported by rocm plugin yet!");
    }
    ...
    for (const auto& op : model_->get_ops()) {
        ...
        rt_info[ov::rocm_gpu::PERF_COUNTER_NAME] = std::make_shared<ov::rocm_gpu::PerfCounts>();
    }

    // Perform any other steps like allocation and filling backend specific memory handles and so on
    const bool opBenchOption = config_.get(ov::rocm_gpu::operation_benchmark.name()).as<bool>();
    const auto creationContext = CreationContext{device, opBenchOption};

    if (use_hip_graph_) {
        auto GraphTopologyRunner = std::make_unique<rocmGraphTopologyRunner>(creationContext, model_);
        number_of_hip_graphs_ = GraphTopologyRunner->GetrocmGraphsCount();
        topology_runner_ = std::move(GraphTopologyRunner);
    } else {
...
size_t CompiledModel::get_optimal_number_of_streams(size_t const_blob_size,
    ...
    throwIfError(hipMemGetInfo(&free, &total));
    ...
    if (0 == available_infer_requests) {
        fprintf(stderr, "[OOM] free=%.1fGB const=%.1fMB mutable=%.1fGB — model requires more VRAM than available\n",
                free / 1073741824.0, const_blob_size / 1048576.0, memory_blob_size / 1073741824.0);
...
std::shared_ptr<ov::ISyncInferRequest> CompiledModel::create_benchmark_sync_infer_request() {
    return std::make_shared<rocmInferRequest>(
...
std::shared_ptr<ov::IAsyncInferRequest> CompiledModel::create_benchmark_infer_request() {
    ...
    return std::make_shared<rocmAsyncInferRequest>(
        std::static_pointer_cast<rocmInferRequest>(std::move(internal_request)),
        get_task_executor(),
        rocm_stream_executor_,
...
std::shared_ptr<ov::IAsyncInferRequest> CompiledModel::create_infer_request() const {
    ...
    return std::make_shared<rocmAsyncInferRequest>(
        std::static_pointer_cast<rocmInferRequest>(internal_request),
        get_task_executor(),
        rocm_stream_executor_,
        get_callback_executor());
}
...
ov::Any CompiledModel::get_property(const std::string& name) const {
    if (ov::supported_properties == name) {
        ...
        supported_properties.push_back(ov::PropertyName(ov::rocm_gpu::number_of_hip_graphs.name(),
    ...
    } else if (ov::rocm_gpu::number_of_hip_graphs == name) {
        return decltype(ov::rocm_gpu::number_of_hip_graphs)::value_type{number_of_hip_graphs_};
    } else {
...
std::shared_ptr<const ov::Model> CompiledModel::get_runtime_model() const {
    ...
    for (const auto& op : model->get_ordered_ops()) {
        ...
        const auto& it = info.find(ov::rocm_gpu::PERF_COUNTER_NAME);
        OPENVINO_ASSERT(it != info.end(), "Operation ", op, " doesn't contain performance counter");
        auto perf_count = it->second.as<std::shared_ptr<ov::rocm_gpu::PerfCounts>>();
...
void CompiledModel::export_model(std::ostream& model_stream) const {
    OV_ITT_SCOPED_TASK(itt::domains::rocm_gpu, "CompiledModel::export_model");
