...
std::vector<ov::PropertyName> Configuration::get_rw_properties() {
    static const std::vector<ov::PropertyName> rw_properties = {
        ...
        ov::PropertyName{ov::rocm_gpu::operation_benchmark.name(), ov::PropertyMutability::RW},
        ov::PropertyName{ov::rocm_gpu::use_hip_graph.name(), ov::PropertyMutability::RW},
    };
...
void Configuration::update_device_id(const ov::AnyMap& config) {
    ...
    if (it != config.end()) {
        ...
        std::smatch match;
        std::regex re_device_id(R"((ROCM\.)?(\d+))");
        if (std::regex_match(value, match, re_device_id)) {
            ...
            if (!device_id_prefix.empty() && "ROCM." != device_id_prefix) {
                throw_ov_exception(
                    fmt::format("Prefix for deviceId should be 'ROCM.' (user deviceId = {}). "
                                "For example: ROCM.0, ROCM.1 and etc.",
                                value));
            }
            ...
            throw_ov_exception(
                fmt::format("Device ID {} is not supported. Supported deviceIds: 0, 1, 2, ROCM.0, ROCM.1, "
                            "ROCM.2 and etc.",
                            value));
        }
    }
...
Configuration::Configuration(const ov::AnyMap& config, const Configuration& defaultCfg, bool throwOnUnsupported) {
    *this = defaultCfg;
    ...
    for (auto&& c : config) {
        ...
        } else if (ov::rocm_gpu::operation_benchmark == key) {
            operation_benchmark = value.as<bool>();
        } else if (ov::rocm_gpu::use_hip_graph == key) {
            use_hip_graph = value.as<bool>();
...
ov::Any Configuration::get(const std::string& name) const {
    ...
    } else if (name == ov::rocm_gpu::operation_benchmark) {
        return operation_benchmark;
    } else if (name == ov::rocm_gpu::use_hip_graph) {
        return use_hip_graph;
