...
PACKAGE_NAME = config('WHEEL_PACKAGE_NAME', 'openvino-rocm')
...
rocm_PLUGIN_CMAKE_TARGET_NAME = 'openvino_rocm_gpu_plugin'
...
class BuildCMakeLib(build_clib):
    def finalize_options(self):
        ...
        self.rocm_plugin_src_dir = os.path.abspath(os.path.dirname(__file__))
        self.build_lib_dir = os.path.join(self.rocm_plugin_src_dir, "../build/lib")
        self.openvino_contrib_src_dir = os.path.normpath(os.path.join(self.rocm_plugin_src_dir, "../../.."))
    ...
    def get_build_env(self):
        ...
        build_env['BUILD_TARGETS'] = rocm_PLUGIN_CMAKE_TARGET_NAME
        ...
        # Update 'rocmCXX' environment variable
        if 'rocmCXX' not in build_env:
            if 'rocm_PATH' in build_env:
                build_env['rocmCXX'] = f"{build_env['rocm_PATH']}/bin/nvcc"
            elif shutil.which("nvcc") is not None:
                build_env['rocmCXX'] = shutil.which("nvcc")

        if 'rocmCXX' not in build_env:
            raise RuntimeError("Cannot detect nvcc compiler path !!"
                               "Please, specify either 'rocmCXX' or 'rocm_PATH' environment variables")

        # Update run-time path to shared library
        rocm_lib_path = os.path.normpath(os.path.join(os.path.dirname(build_env['rocmCXX']), "../lib64"))
        python_lib_path = os.path.normpath(os.path.join(os.path.dirname(sys.executable), "../lib"))
        libraries_path = rocm_lib_path + os.pathsep + python_lib_path
    ...
    def build_rocm_plugin(self):
        if not os.path.isdir(self.openvino_build_dir):
            self.mkpath(self.openvino_build_dir)

        build_env = self.get_build_env()

        self.announce("Building OpenVINO rocm Plugin Project", level=3)
        run_command([os.path.join(self.rocm_plugin_src_dir, "../build.sh"), '--build'],
                    cwd=self.rocm_plugin_src_dir, env=build_env)

    def locate_built_lib(self):
        libs = []
        lib_ext = platform_specifics.get_lib_file_extension()
        bin_dir = os.path.join(self.openvino_src_dir, "bin")
        for name in [rocm_PLUGIN_CMAKE_TARGET_NAME]:
            libs.extend(list(glob.iglob(f"{bin_dir}/**/*{name}*{lib_ext}", recursive=True)))
        if not libs:
            raise Exception("rocm Plugin library not found. Possibly build was failed or was written to unknown "
                            "directory")
...
class InstallCMakeLib(install_lib):
    ...
    def run(self):
        try:
            ...
            self.register_rocm_plugin()
            self.test_rocm_plugin()
            openvino_rocm_gpu_library = f'{openvino_src_dir}/bin/intel64/Release/lib/libopenvino_rocm_gpu_plugin.{platform_specifics.get_lib_file_extension()}'
            package_data.update({
                '': [openvino_rocm_gpu_library]
            })
        finally:
            self.unregister_rocm_plugin()
    ...
    def register_rocm_plugin(self):
        openvino_package_libs_dir = self.get_openvino_package_dir()
        self.copy_tree(self.build_dir, openvino_package_libs_dir)
        openvino_rocm_gpu_library = os.path.join(openvino_package_libs_dir,
                                                   f"libopenvino_rocm_gpu_plugin.{platform_specifics.get_lib_file_extension()}")

        xml_file = os.path.join(openvino_package_libs_dir, "plugins.xml")
        tree = ET.parse(xml_file).getroot()
        plugins = tree.find("plugins")
        if all(plugin.get('name') != 'rocm' for plugin in plugins.iter('plugin')):
            plugins.append(Element('plugin', {'name': 'rocm', 'location': openvino_rocm_gpu_library}))
            with open(xml_file, "w") as f:
                f.write(ET.tostring(tree).decode('utf8'))

    def unregister_rocm_plugin(self):
        openvino_package_libs_dir = self.get_openvino_package_dir()
        ...
        for plugin in plugins.iter('plugin'):
            if plugin.get('name') == 'rocm':
                plugins.remove(plugin)
                with open(xml_file, "w") as f:
                    f.write(ET.tostring(tree).decode('utf8'))
                break

    def test_rocm_plugin(self):
        ...
        try:
            core.compile_model(model=model, device_name="rocm")
            ...
            raise RuntimeError('The rocm GPU plugin loading test was failed. '
                               'The rocm GPU plugin library is not compatible with OpenVINO libraries. '
...
setup(
    ...
    description=config('WHEEL_DESC', 'rocm Plugin for OpenVINO Inference Engine Python* API'),
    ...
    packages=["openvino_rocm"],
    package_dir={
        "openvino_rocm": f"{os.path.abspath(os.path.dirname(__file__))}/openvino_rocm",
    },
