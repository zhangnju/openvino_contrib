...
std::vector<std::string> disabledTestPatterns() {
    ...
    if (!rocm::isHalfSupported(rocm::Device{})) {
        retVector.emplace_back(
            R"(.*OVExecGraphImportExportTest.*importExportedFunctionParameterResultOnly.*targetDevice=rocm_elementType=f16.*)");
        retVector.emplace_back(
            R"(.*OVExecGraphImportExportTest.*importExportedIENetworkParameterResultOnly.*targetDevice=rocm_elementType=f16.*)");
    }
