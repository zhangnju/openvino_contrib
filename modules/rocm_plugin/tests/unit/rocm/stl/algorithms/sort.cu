...
__global__ void quick_sort_launch(T* first, T* last, int* stack, Compare comparer) {
    rocm::algorithms::quick_sort_iterative(first, last, stack, comparer);
}

template <typename T, typename Compare>
__global__ void quick_sort_launch(T* first, T* last, Compare comparer) {
    rocm::algorithms::quick_sort_iterative(first, last, comparer);
}

template <typename T, typename Compare>
__global__ void partial_quick_sort_launch(T* first, T* last, int* stack, const size_t topk, Compare comparer) {
    rocm::algorithms::partial_quick_sort_iterative(first, last, stack, topk, comparer);
}

template <typename T, typename Compare>
__global__ void partial_quick_sort_launch(T* first, T* last, const size_t topk, Compare comparer) {
    rocm::algorithms::partial_quick_sort_iterative(first, last, topk, comparer);
}
...
__global__ void parallel_quick_sort_launch(T* first, T* last, int* stack, Compare comparer) {
    ...
    rocm::algorithms::parallel::quick_sort_iterative<kNumPartitions>(
...
__global__ void parallel_quick_sort_launch(T* first, T* last, Compare comparer) {
    ...
    constexpr auto kNumPartitions = 1024;
    rocm::algorithms::parallel::quick_sort_iterative<kNumPartitions>(begin, end, comparer, threadIdx.x, blockDim.x);
}
...
public:
    ...
    void runQuickSort() {
        ...
        if (WithStack) {
            ...
            quick_sort_launch<<<1, 1, 0, stream.get()>>>(begin, end, stack_begin, rocm::algorithms::Less<float>{});
        } else {
            quick_sort_launch<<<1, 1, 0, stream.get()>>>(begin, end, rocm::algorithms::Less<float>{});
        }
        stream.synchronize();
    ...
    void runPartialQuickSort() {
        ...
        if (WithStack) {
            ...
            partial_quick_sort_launch<<<1, 1, 0, stream.get()>>>(
                begin, end, stack_begin, TopK, rocm::algorithms::Less<float>{});
        } else {
            partial_quick_sort_launch<<<1, 1, 0, stream.get()>>>(begin, end, TopK, rocm::algorithms::Less<float>{});
        }
        stream.synchronize();
        defaultStream.download(sortedArr.get(), src, arr_size);
    ...
    void runParallelQuickSort() {
        ...
        if (WithStack) {
            ...
            parallel_quick_sort_launch<<<1, 100, 0, stream.get()>>>(
                begin, end, stack_begin, rocm::algorithms::Less<float>{});
        } else {
            parallel_quick_sort_launch<<<1, 100, 0, stream.get()>>>(begin, end, rocm::algorithms::Less<float>{});
        }
        stream.synchronize();
        defaultStream.download(sortedArr.get(), src, arr_size);
