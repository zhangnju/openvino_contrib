...
__global__ void verify_extents(rocm::MDSpan<int, rocm::DExtents<2>> span) {
...
__global__ void assign(rocm::MDSpan<int, rocm::DExtents<2>> span) {
    assert(span.extent(0) == 100);
    assert(span.extent(1) == 512);
    assert(gridDim.x == 100);
    assert(blockDim.x == 512);
...
__global__ void verify(rocm::MDSpan<int, rocm::DExtents<2>> span) {
    assert(span.extent(0) == 100);
    assert(span.extent(1) == 512);
    assert(gridDim.x == 100);
    assert(blockDim.x == 512);
...
TEST_F(MDSpanTest, MDSpan_VerifyExtents) {
    using MDSpanTestType = rocm::MDSpan<int, rocm::DExtents<2>>;
