...
__global__ void push_back(rocm::Vector<T> vec) {
...
template <size_t VectorCapacity, typename T>
__global__ void verify_push_back(rocm::Vector<T> vec) {
    assert(vec.size() == VectorCapacity);
}

template <size_t VectorCapacity, typename T>
__global__ void erase(rocm::Vector<T> vec) {
...
template <size_t VectorCapacity, typename T>
__global__ void verify_erase(rocm::Vector<T> vec) {
...
TEST_F(VectorTest, Vector_PushBack) {
    using VectorTestType = rocm::Vector<int>;
    ...
    auto vec = rocm::Vector<int>(src.get(), kVectorCapacity);
