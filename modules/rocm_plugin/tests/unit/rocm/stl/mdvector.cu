...
__global__ void push_back(rocm::MDVector<T, 2> mdvec) {
    auto vec = mdvec(0, 1);
...
template <size_t VectorCapacity, typename T>
__global__ void verify_push_back(rocm::MDVector<T, 2> mdvec) {
    auto vec0 = mdvec(0, 0);
    auto vec1 = mdvec(0, 1);
    assert(vec0.size() == 0);
...
template <size_t VectorCapacity, typename T>
__global__ void erase(rocm::MDVector<T, 2> mdvec) {
    auto vec = mdvec(0, 1);
...
template <size_t VectorCapacity, typename T>
__global__ void verify_erase(rocm::MDVector<T, 2> vec) {
...
TEST_F(MDVectorTest, MDVector_PushBack) {
    using VectorTestType = rocm::MDVector<int, 2>;
