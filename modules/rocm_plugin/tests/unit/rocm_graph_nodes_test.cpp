...
TEST_F(KernelNodeTest, InsertKernel) {
    ...
    auto insert = kernel::Insert(element_type, props, rocm::Device{}.props().maxThreadsPerBlock);
    ...
    std::optional<rocm::KernelNode> kernel_node;
    ...
    rocm::GraphExec graph_exec{capture.getGraph()};
...
TEST_F(TransferNodeTest, Transfer) {
    ...
    std::optional<rocm::TransferNode> transfer_node;
    rocm::GraphCapture capture{stream};
    {
        auto scope = capture.getScope();
        rocm::CaptureInfo captureInfo{stream};
