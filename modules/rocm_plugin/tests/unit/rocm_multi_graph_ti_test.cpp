...
public:
    ...
    static void checkContext(rocmGraphContext& rocmGraphContext) {
        // TI has always a separate graph in rocmGraphContext
        // Single-graph TI version uses rocmGraphInfo object with 1 graph
        // Total graph count should be 3
        EXPECT_EQ(rocmGraphContext.get_graphs_count(), 3);
        EXPECT_TRUE(rocmGraphContext.is_nested());

        rocmGraphContext.select_current_graph(1);
        const auto& tiGraph = rocmGraphContext.get_current_graph();
        EXPECT_FALSE(tiGraph.is_nested());
        EXPECT_EQ(tiGraph.get_graphs_count(), 1);
    }

    static void checkRunner(const rocmGraphTopologyRunner& runner) {
        // rocmGraphTopologyRunner always puts a TI into a separate SubGraph
        // Single-graph TI version doesn't use nested rocmGraphTopologyRunner objects and uses 1 graph
        // Total graph count should be 3
        EXPECT_EQ(runner.GetSubGraph().GetrocmGraphCompatibility(), rocmGraphCompatibility::SPECIAL);
        EXPECT_EQ(runner.GetrocmGraphsCount(), 3);
...
public:
    ...
    static void checkContext(rocmGraphContext& rocmGraphContext) {
        // TI has always a separate graph in rocmGraphContext
        // Multi-graph TI version uses rocmGraphPack object with 3 graphs
        // Total graph count should be 5
        EXPECT_EQ(rocmGraphContext.get_graphs_count(), 5);
        EXPECT_TRUE(rocmGraphContext.is_nested());
        rocmGraphContext.select_current_graph(1);
        const auto& tiGraph = rocmGraphContext.get_current_graph();
    ...
    static void checkRunner(const rocmGraphTopologyRunner& runner) {
        // rocmGraphTopologyRunner always puts a TI into a separate SubGraph
        // Multi-graph TI version uses nested rocmGraphTopologyRunner and uses 3 graphs
        // Total graph count should be 5
        EXPECT_EQ(runner.GetSubGraph().GetrocmGraphCompatibility(), rocmGraphCompatibility::SPECIAL);
        EXPECT_EQ(runner.GetrocmGraphsCount(), 5);
        EXPECT_TRUE(runner.hasNestedRunners());
    }
};

template <typename Network>
class rocmMultiGraphTest : public Test {
protected:
    ...
    void checkConditions() {
        Network::checkContext(rocmGraphContext_);
...
using GRUTIMultiGraphTest = rocmMultiGraphTest<GRUTI>;

TEST_F(GRUTIMultiGraphTest, rocmMultiGraphTest) { runTest(); }

using SplitConcatAddTIMultiGraphTest = rocmMultiGraphTest<SplitConcatAddTI>;

TEST_F(SplitConcatAddTIMultiGraphTest, rocmMultiGraphTest) { runTest(); }
