...
using CDevPtr = rocm::DevicePointer<const void*>;

struct rocmGraphCompatibilityTest : testing::Test {
    ...
    static bool checkAndRun(const OperationBase::Ptr& operation,
        ...
        if (operation->GetrocmGraphCompatibility() == rocmGraphCompatibility::FULL) {
            stream.synchronize();
            ...
            stream.synchronize();
            std::cout << "--- Operation compatible. Running with rocmGraph ---\n";
            exec.launch(stream);
            return true;
        }
        std::cout << "--- Operation isn't compatible. Running without rocmGraph ---\n";
        operation->Execute(context, inputs, outputs, workbuffers);
        return false;
    }
};

struct RelurocmGraphCompatibilityTest : rocmGraphCompatibilityTest {
    void run() {
        ...
        rocm::Allocation inAlloc = stream.malloc(inSizeBytes);
        rocm::Allocation outAlloc = stream.malloc(outSizeBytes);
        ...
        // Run with or without rocmGraph usage
...
TEST_F(RelurocmGraphCompatibilityTest, Compatibile) { run(); }

struct ConcatrocmGraphCompatibilityTest : rocmGraphCompatibilityTest {
    void run() {
        using ElementType = float;
        ...
        ThreadContext threadContext{device};
        auto& stream = threadContext.stream();
        rocm::Allocation inAlloc1 = stream.malloc(inSizeBytes1);
        rocm::Allocation inAlloc2 = stream.malloc(inSizeBytes2);
        rocm::Allocation outAlloc = stream.malloc(outSizeBytes);
...
TEST_F(ConcatrocmGraphCompatibilityTest, NotCompatible) { run(); }
