...
public:
    ...
    static void checkContext(const rocmGraphContext& rocmGraphContext) {
        // AddMul network should have a single rocm Graph
        EXPECT_EQ(rocmGraphContext.get_graphs_count(), 1);
    }

    static void checkSubGraph(const SubGraph& subGraph) {
        // Original SubGraph for AddMul network should be rocm Graph compatible
        EXPECT_EQ(subGraph.GetrocmGraphCompatibility(), rocmGraphCompatibility::FULL);
    }
...
public:
    ...
    static void checkContext(const rocmGraphContext& rocmGraphContext) {
        // AddConcat network should have a more than one rocm Graph
        EXPECT_GT(rocmGraphContext.get_graphs_count(), 1);
    }

    static void checkSubGraph(const SubGraph& subGraph) {
        // Original SubGraph for AddConcat network should not be rocm Graph compatible
        EXPECT_EQ(subGraph.GetrocmGraphCompatibility(), rocmGraphCompatibility::NONE);
    }
...
using AddMulMultiGraphTest = rocmMultiGraphTest<AddMul>;

TEST_F(AddMulMultiGraphTest, AddMulTest) { runTest(); }

using AddConcatMultiGraphTest = rocmMultiGraphTest<AddConcat>;
