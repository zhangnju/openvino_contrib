...
TEST_P(MatMulCompileModelTest, BuildExecutableSequence_MatMul_Success) {
    ...
    auto rocm_compiled_model = std::dynamic_pointer_cast<CompiledModel>(plugin->compile_model(model_, properties));
    const auto& execSequence = GetExecSequence(rocm_compiled_model);
...
TEST_P(NumStreams1CompileModelTest, CompileModel_OptimalNumberInferRequests_1_Success) {
    ...
    auto rocm_compiled_model = std::dynamic_pointer_cast<CompiledModel>(compiled_model);
    auto& memoryManagerPool = GetMemoryManagerPool(rocm_compiled_model);
    ASSERT_EQ(memoryManagerPool->Size(), total_streams);
    ASSERT_EQ(rocm_compiled_model->get_property(ov::num_streams.name()), ov::streams::Num(total_streams));
    ASSERT_EQ(rocm_compiled_model->get_property(ov::optimal_number_of_infer_requests.name()), uint32_t(total_streams));
}
