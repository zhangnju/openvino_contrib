...
template <class NGraphPoolingNode, class MIOPENPoolingNode>
class PoolingRegistryTest : public testing::Test {
    void SetUp() override {
        rocm::Device device{};
        const bool optimizeOption = false;
        // A rocm Plugin's Op registration requires at least 1 explicit
        // construction of the node.
        auto ngraph_node_dummy = build_ngraph_pooling_dummy<NGraphPoolingNode>();
        MIOPENPoolingNode registration_dummy(CreationContext{device, optimizeOption},
                                            ngraph_node_dummy,
...
struct PoolingTest : testing::Test {
    void initializerocmBuffers(gsl::span<const float> in, gsl::span<const float> out) {
    ...
    void test(gsl::span<const float> input, std::vector<size_t> in_shape, gsl::span<const float> output) {
        ...
        initializerocmBuffers(input, output);
        ...
        rocmMemcpy(result, outputs[0].get(), output.size_bytes(), rocmMemcpyDeviceToHost);
    ...
    std::vector<rocm::Allocation> allocs;
    std::vector<rocm::DevicePointer<const void*>> inputs;
    std::vector<rocm::DevicePointer<void*>> outputs;
