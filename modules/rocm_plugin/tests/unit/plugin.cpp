...
TEST_F(PluginTest, CompileModel_OutRangeId_Failed) {
    auto dummyFunction = std::make_shared<ov::Model>(ov::NodeVector{}, ov::ParameterVector{});
    auto plugin = std::make_shared<Plugin>();
    ASSERT_THROW(plugin->compile_model(dummyFunction, {{ov::device::id.name(), std::to_string(rocm::Device::count())}}),
                 ov::Exception);
}

TEST_F(PluginTest, CompileModel_rocmThreadPool_Success) {
    using namespace std::chrono_literals;

    auto plugin = std::make_shared<Plugin>();
    auto execNetwork = plugin->compile_model(model_, {{ov::device::id.name(), "0"}});
    const int deviceId = 0;
    rocm::Device device{deviceId};
    auto cpuStreamExecutor = std::make_shared<rocmThreadPool>(device, max_concurrent_streams(device));
...
TEST_F(PluginTest, CompileModel_rocmThreadPool_AllJobs_Success) {
    using namespace std::chrono_literals;
    ...
    auto cpuStreamExecutor = std::make_shared<rocmThreadPool>(device, numConcurrentStreams);
...
TEST_F(PluginTest, CompileModel_rocmThreadPool_AllJobs_Heavy_Success) {
    using namespace std::chrono_literals;
