...
TEST(TransformationTests, rocm_transformations_f16) {
    std::shared_ptr<ov::Model> model, model_ref;
    {
        ...
        // Run transformation
        const rocm::Device device{};
        const auto config = ov::rocm_gpu::Configuration(ov::AnyMap{ov::hint::inference_precision(ov::element::f16)});
        ov::rocm_gpu::GraphTransformer().transform(device, model, config);
        ...
        if (!rocm::isHalfSupported(device)) {
