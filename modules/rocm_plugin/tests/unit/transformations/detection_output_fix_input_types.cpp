...
ov::rocm_gpu::OperationBase::Ptr createPluginOperation(const std::shared_ptr<ov::Node>& node) {
    return ov::rocm_gpu::OperationRegistry::getInstance().createOperation(
        ov::rocm_gpu::CreationContext{rocm::Device{}, false},
        node,
...
TEST(detection_output_fix_input_types, three_params) {
    ...
    std::shared_ptr<Model> model, model_ref;
    {
        ...
        pass_manager.register_pass<rocm_gpu::pass::DetectionOutputFixInputTypesTransformation>();
