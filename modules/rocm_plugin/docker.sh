...
[[ "$1" == "install" || "$1" == "build" || "$1" == "run" ]] || {
  ...
  - install (allow to install docker with rocm-toolkit support)\n\
...
function install() {
    ...
    curl -s -L https://rocm.github.io/rocm-docker/gpgkey | sudo apt-key add -
    curl -s -L https://rocm.github.io/rocm-docker/$distribution/rocm-docker.list | sudo tee /etc/apt/sources.list.d/rocm-docker.list

    sudo apt-get update
    sudo apt-get install -y rocm-container-toolkit
...
function build() {
    if [[ ! ./Dockerfile -ef ${rocm_PACKAGES_PATH}/Dockerfile ]]; then
        cp Dockerfile "${rocm_PACKAGES_PATH}"/
    fi

    pushd "${rocm_PACKAGES_PATH}"
    docker build -t openvino/rocmplugin-2022.3 .
    if [[ ! ./Dockerfile -ef ${rocm_PACKAGES_PATH}/Dockerfile ]]; then
        rm -f "${rocm_PACKAGES_PATH}"/Dockerfile
    fi
    popd
}
...
function run() {
    ...
    docker run --gpus all -e LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" \
                          ...
                          -w "${PWD}" openvino/rocmplugin-2022.3 \
                          ${USER_APP}
}
...
if [[ "$1" == "install" ]]; then
  ...
  [[ -n ${rocm_PACKAGES_PATH} ]] || { echo "rocm_PACKAGES_PATH environment variable is expected"; exit 1; }
  build
elif [[ "$1" == "run" ]]; then
